...
public class SectionDecoration extends RecyclerView.ItemDecoration {
    ...
    public SectionDecoration(List<Person> dataList, Context context, DecorationCallback decorationCallback) {
        Resources res = context.getResources();
        this.dataList = dataList;
        this.callback = decorationCallback;
        setPullAction(dataList);
        //设置悬浮栏的画笔---paint
        paint = new Paint();
    ...
    private void setPullAction(List<Person> comingslist) {
        dataList = new ArrayList<>();

        for (int i = 0; i < comingslist.size(); i++) {
            Person nameBean = new Person();
            String name0 = comingslist.get(i).comingTitle;;
            nameBean.comingTitle=name0;;
